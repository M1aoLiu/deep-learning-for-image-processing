...
class VOCSegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        # 注意，在语义分割中对应的标注图像（.png）用PIL的Image.open()函数读取时，默认是P模式，即一个单通道的图像。
        # 在背景处的像素值为0，目标边缘处用的像素值为255(训练时一般会忽略像素值为255的区域)，
        # 目标区域内根据目标的类别索引信息进行填充，例如人对应的目标索引是15，所以目标区域的像素值用15填充。
        # (382, 500, 3) ,(382, 500)
