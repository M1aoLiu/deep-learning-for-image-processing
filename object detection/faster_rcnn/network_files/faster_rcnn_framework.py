...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 ...
                 # RPN parameters
                 rpn_anchor_generator=None, rpn_head=None, # 前者用于生成anchor，后者表示RPN中的3*3和两个1*1
                 ...
                 # （anchor与GT的IOU大于0.7为正样本，与任一GT小于0.3为负样本）
                 rpn_batch_size_per_image=256, rpn_positive_fraction=0.5,  # rpn计算损失时采样的样本数，以及正样本占总样本的比例
                 rpn_score_thresh=0.0,
                 # Box parameters
                 # 分别表示ROIPooling，Flatten+两个FC 以及最终的两个FC预测
                 box_roi_pool=None, box_head=None, box_predictor=None,
                 # 移除低目标概率      fast rcnn中进行nms处理的阈值   对预测结果根据score排序取前100个目标
                 box_score_thresh=0.05, box_nms_thresh=0.5, box_detections_per_img=100,
                 box_fg_iou_thresh=0.5, box_bg_iou_thresh=0.5,   # fast rcnn计算误差（区别前面的RPN的正负样本）时，采集正负样本设置的阈值
